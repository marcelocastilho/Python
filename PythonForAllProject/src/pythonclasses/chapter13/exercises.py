...
#Actual data: http://py4e-data.dr-chuck.net/comments_119190.xml (Sum ends with 0)
...
url = 'http://py4e-data.dr-chuck.net/comments_119190.xml'
print('Using the url:', url)

xmlFile = urllib.request.urlopen(url, context=ctx).read()
#print(xmlFile)

xml = ET.fromstring(xmlFile)

comments = xml.findall('comments/comment')
print('Number of comments:', len(comments))


print('Printing comments list:', comments,'\n')

#for comment in comments:
#  print('Name:', comment.find('name').text)
#  print('Id:', comment.find('count').text)

# using XPath
print( sum([int(count.text) for count in xml.findall('.//count')])) 
